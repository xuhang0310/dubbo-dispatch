...
public class DataServiceImpl extends GridSqlUtilTool implements IDataService{
	...
	public String getSql(Map<String, String> paramMap){
		String sql1="select CODE,ID,STIME,CONTENT,sdate,SCHEDULERID,SORDER,orgname,STYPENAME,   case when STATE = 1 then '未完成' when STATE = 0 then '完成' end as STATE,STYPE,EMERY,RECEIVEOBJECT ,TEAMCODE,RECEIVEUSER ,FINISHEDDATE,FINISHTHING,teamid,schedulername from (";
		...
		sql2+=" [ and to_date(sdate,'yyyy-mm-dd') <= to_date('{enddate}','yyyy-mm-dd') ] ";
		 ...
		 /*if(null!=dsdlo.getStype()&&!dsdlo.getStype().equals("")){
			 sql2+="  and  temp.stype='"+dsdlo.getStype()+"'";
		 }
		 if(null != dsdlo.getBzmc() && !dsdlo.getBzmc().equals("")){
			 sql2 += " and temp.teamid='"+dsdlo.getBzmc()+"'";
		 }*/
		 /*if(null!=dsdlo.getEmery()&&!"".equals(dsdlo.getEmery()))	
			 sql2+=" and EMERY like '%"+dsdlo.getEmery()+"%'"  ;*/
		 ...
		 /*if(null!=dsdlo.getState()&&!"".equals(dsdlo.getState())){
			 sqlimp+=" and  4="+dsdlo.getState();	 
		}
		 if(null != dsdlo.getBzmc() && !dsdlo.getBzmc().equals("")){
			 sqlimp += " and s.teamid='"+dsdlo.getBzmc()+"'";
		 }
		 if (!Public.isEmpty(pageConfig.getSortCol())) {
			 sqlimp += " ORDER BY " + pageConfig.getSortCol() + " " + pageConfig.getAscOrDesc() + " ";
			} else {
				sqlimp += " order by sdate desc ";
			}*/
	...
	public List findDqZbr(Map<String, String> paramMap) throws Exception{
		String sql = "select t2.schedulername,t1.* from" 
	            ...
	            +" where t1.TEAMID=t2.TEAMID and t2.parentorg in (SELECT ORGID FROM SYS_USER WHERE USERNAME IN ('"+paramMap.get("LoginName")+"'))";
	...
	public List getZbrListByWatchcode(String watchcode) throws Exception{
		...
		if(!watchcode.isEmpty()){
			...
			String ursql = "SELECT * FROM SYS_USER T WHERE USERNAME IN (";
			String str = "";
