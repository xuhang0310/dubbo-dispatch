...
public class ApplyDispatchServiceImpl extends GridSqlUtilTool implements IApplyDispatchService{
	...
	public String getCommonSql(Map<String, String> paramMap){
		String commonsql = "select t.taskorderid,(select orgname from sys_org where orgid=t.EXECUTEORGID) executeorgname, "+
				...
				"          '重要' "+
				"       end) importantname, "+
				"		decode(t.curstate,'0','草稿','1','审批中',2,'结束') curstatename, "+
				"       b.name  typevalue,t.id idopreat "+
				"  from task_apply_flow t  "+
				"  left join bas_powertype b "+
				"    on t.tasktype = b.id "+
				" where t.del = 1 ";//and t.writeor='"+pageConfig.getUid()+"'     and t.executeorgid in ("+pageConfig.getUserDataPermissions()+")   
	...
	public String getDaiBanObject(Map<String, String> paramMap){
		String sql="select t.taskorderid, "+
				...
				"from task_apply_flow t, sys_user su "+//task_flow_next tfn, 
				"where t.del = 1 "+
				"   and t.CURSTATE = '1' ";
				/*"   and t.CURSTATE <> '0' ";*/
				//"   and tfn.taskid = t.id "+
	...
	public String getYiBanObject(Map<String, String> paramMap){
		String sql="select t.taskorderid, "+
				...
				"from task_apply_flow t, ( select *  from task_flow_log where (taskid,userid, flow_overtime) " +
	...
	public List getDaiShenPiList(String id) throws Exception {
		// TODO Auto-generated method stub
		String sql="select taskflowid,executeorgid from task_apply_flow  t where id='"+id+"' ";
		List list=baseDao.findAll(sql);
		...
		if(listYiShenPi.size()!=0){
			...
			for(int i=0;i<listShenPiAll.size();i++){
				...
				if(bool&&datapermissions.indexOf(executeorgid)!=-1){
					if(hasThisObj(groupCodeTemp,groupCodeList)){
	...
	public boolean hasThisObj(String groupCode,List list){
		boolean bool=true;
		if(list.size()==0){
			bool=true;
			return bool;
