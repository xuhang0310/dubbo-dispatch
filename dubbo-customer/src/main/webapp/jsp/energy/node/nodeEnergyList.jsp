...
<html>
  ...
  <body style="margin:10px;overflow-x:hidden;">
	    ...
	    <div class="row">
			...
			<form id="listForm" name="listForm" action="#">	
			 <input type="hidden" value="" name="chartTitle" id="chartTitle"/>
			    ...
			    <select class="selectpicker" data-live-search="true"  name="orgid" id="orgid" onchange="getProjectList()" title="请选择公司" >
		              <option value="">--请选择--</option>
		             <c:forEach var="temp" items="${sysorg }">
		             		 <option value="${temp.ORGID }">${temp.ORGNAME }</option>
		             </c:forEach>
	            </select>
				<select class="selectpicker" data-live-search="true" multiple  name="nodename" id="nodename" title="请选择换热站" >
		             <c:forEach var="temp" items="${node }">
		             		 <option value="${temp.NODEID }">${temp.NODENAME }</option>
		             </c:forEach>
	            </select>
				...
				<a href="javascript:layer_show('能耗数据图例','<%= request.getContextPath() %>/base/legendChart.do?pageid=3002&chartField='+$('#chartField').val(),'600','200')" class="btn btn-primary radius"  ><i class="icon-bar-chart"></i> 图例</a>
			<!-- 	<button onclick="search()" class="btn btn-success"  name="b"><i class="icon-glass"></i> 同期对比</button> -->
	      ...
	      <div id="pie"  style="width:100%;height:300px;"></div>
	         ...
	         <script type="text/javascript">
								    ...
								    function getLineChart(){
								    	...
								    	$.ajax({
											  type: 'POST',
											  url: "<%=path%>/nodeEnergy/getNodeEnergyChart.do",
											  data: datas,
						    	...
						    	$.ajax({
									  type: 'POST',
									  url: "<%=path%>/nodeEnergy/getNodeEnergyPieChart.do",
									  data: datas,
									  success: function(result){
    ...
    <div id="allmap"  class="row" style="margin-top: 10px;">
	 	<div class="col-sm-12">	 	
	 	<div class="analyse-chart">
	  <iframe src="<%=path %>/mapMaintenance/NodeMapLayer.do" id="mainiframe" width="100%" height="400px" scrolling="no" frameborder="0"></iframe>
		 </div>
	  ...
	  $('#nhlx').selectpicker({
		 ...
		 var columns =eval(${jsonTableGrid});
		    $table.bootstrapTable({
		        ...
		        method: 'post',
		        url:"<%=path%>/nodeEnergy/getNodeEnergyList.do",
		        dataType: "json",
		        striped: true,
		        pagination: true,
	...
	function responseHandler(res) {
	 ...
	 function queryParams(params) {
		 ...
		 var datas= $.param({rows: params.limit,page: params.pageNumber,nodecode:$("#nodename").val()+""}) + '&' + $('#listForm').serialize();
		 getSummary();
		 return datas;
	} 
		...
		$.ajax({
			  type: 'POST',
			  url: "<%=path%>/nodeEnergy/getNodeSummaryEnergyList.do",
			  data: datas,
			  success: function(result){
	...
	function exportExcel(){
		// var datas= $.param({nodecode:$("#nodename").val()+""}) + '&' + $('#listForm').serialize();
		location.href="<%=path%>/nodeEnergy/exportExcel.do?pageid=3002&"+$.param({nodecode:$("#nodename").val()+""}) + '&' + $('#listForm').serialize();
	}
	
	function showAssess(){
	var url = "<%=path%>/nodeEnergy/showAssess.do?pageid=300201";
		window.parent.addTabs({id:'nodeenergy99-4',title: '能耗评估',close: true,url: url});
	}
	
	//能耗梯度
	function energyGradient(){
	layer_show('修改',"<%= request.getContextPath() %>/nodeEnergy/energyGradient.do",'400','350')
