    ...
    <head>
       ...
       <!-- ace styles -->
        ...
        var setting = {
                ...
                async: {    //ztree异步请求数据 
                    enable: true, 
                    url: "<%=path%>/dataDic/getDataDicTree.do",//请求action方法 
                    autoparam:["id"] 
                },
                callback:{
...
<div class="row">     
    ...
    <div class="row">
		<div class="col-sm-12">
			<form id="listForm" name="listForm" action="#">	
			 ...
			 <input type="text" class="input-text" style="width:250px" placeholder="请输入数据名称" id="name" name="name">
			 <span><a class="btn btn-success radius" href="javascript:search()"><i class="icon-search"></i>查询</a></span>
			<shiro:hasPermission name="/jsp/data/addDataDic">
			  <span><a class="btn btn-primary radius"  onclick="layer_show('添加数据字典','<%= request.getContextPath() %>/dataDic/addDataDicList.do','500')"><i class="icon-edit"></i> 添加数据字典</a> </span>
            </shiro:hasPermission>
            </form>
		</div>
    </div>
	...
	$(function () {
	    ...
	    $table.bootstrapTable({
	        ...
	        method: 'post',
	        url:"<%=path%>/dataDic/getDataDicList.do",
	        dataType: "json",
	        striped: true,
	        pagination: true,
	...
	function operateFormatter(value, row, index) {
		 var temp= '';
		 <shiro:hasPermission name="/jsp/data/updateDataDic">
		    temp +="<a class=\"like\" href=\"javascript:editDataDic(\'"  +value+   "\')\" title=\"编辑\"><i class=\"icon-edit\"></i>编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;   ";
         </shiro:hasPermission>
         
         <shiro:hasPermission name="/jsp/data/deleteDataDic">
		    //temp += '  <a class="like" href="javascript:deleteFeed('+value+')" title="删除"><i class="icon-remove"></i></a>';
		    temp +="<a class=\"like\" href=\"javascript:deleteDataDic(\'"  +value+   "\')\" title=\"删除\"><i class=\"icon-remove\"></i>删除</a>";
         </shiro:hasPermission>
		
		 return temp;
    }
	
	function editDataDic(obj){
		
		layer_show('修改数据',"<%= request.getContextPath() %>/dataDic/updateDataDicList.do?id="+obj,'500')
	}
	
	function deleteDataDic(obj){
		
		layer.confirm('确认删除吗?',{btn:['是','否']},function(){
			 var url="<%=path %>/dataDic/deleteDataDic.do?id="+obj;
