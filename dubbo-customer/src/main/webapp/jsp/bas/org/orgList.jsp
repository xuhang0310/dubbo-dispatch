...
<div class="row">
		...
		<div class="row">
				<div class="col-sm-12"> 组织机构名称：
					<input type="hidden" value="" id="orgid"/>
					 <input type="text" class="input-text" style="width:250px" placeholder="输入关键词" id="orgname" name="orgname">
					...
					<shiro:hasPermission name="/jsp/org/addOrg">
					  <span><a class="btn btn-primary radius"  onclick="layer_show('添加组织结构','<%= request.getContextPath() %>/jsp/user/eidtUser.jsp','600')"><i class="icon-edit"></i> 添加组织结构</a> </span>
		            </shiro:hasPermission>
				</div>
		    </div>
	...
	function responseHandler(res) {
	 ...
	 function queryParams(params) {
		return {rows: params.limit,page: params.pageNumber,orgname:$("#orgname").val(),orgid:$("#orgid").val()};
	} 
	function getHeight() {
	...
	function editUser(obj){
		...
		layer_show('修改','<%= request.getContextPath() %>/jsp/user/eidtUser.jsp','500')
	}
