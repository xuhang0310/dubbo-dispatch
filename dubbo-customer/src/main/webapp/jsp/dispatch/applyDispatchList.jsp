  ...
  <body style="margin:10px;overflow:hidden;" >
    ...
    <div class="row">
		<div class="col-sm-12">
			...
			<%-- <shiro:hasPermission name="/jsp/dispatch/addApplyDispatch">
			  <span><a class="btn btn-primary radius"  onclick="layer_show('新建','<%= request.getContextPath() %>/dataTeam/addDataTeamList.do','500')"><i class="icon-edit"></i> 新建</a> </span>
            </shiro:hasPermission> --%>
            </form>
		</div>
    </div>
	...
	$(function () {
	    ...
	    $table.bootstrapTable({
	        ...
	        method: 'post',
	        url:"<%=path%>/applyDispatch/getDispatchList.do",
	        dataType: "json",
	        striped: true,
	        pagination: true,
	...
	function operateFormatter(value, row, index) {
		 var temp= '';
		 <shiro:hasPermission name="/jsp/dispatch/updateApplyDispatch">
		    temp +="<a class=\"like\" href=\"javascript:editApplyDispatch(\'"  +value+   "\')\" title=\"编辑\"><i class=\"icon-edit\"></i>编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;   ";
         </shiro:hasPermission>
         
         <shiro:hasPermission name="/jsp/dispatch/deleteApplyDispatch">
		    //temp += '  <a class="like" href="javascript:deleteFeed('+value+')" title="删除"><i class="icon-remove"></i></a>';
		    temp +="<a class=\"like\" href=\"javascript:deleteApplyDispatch(\'"  +value+   "\')\" title=\"删除\"><i class=\"icon-remove\"></i>删除</a>";
         </shiro:hasPermission>
	...
	function showScheduler(value) {
    	...
    	layer_show('申请令流程跟踪',"<%= request.getContextPath() %>/applyDispatch/getJqueryZCLCTList.do?taskId="+ value,'500');
   	}
    
	function editApplyDispatch(obj){
		
		layer_show('修改申请令',"<%= request.getContextPath() %>/applyDispatch/updateApplyDispatchList.do?id="+obj,'500')
	}
	
	function deleteApplyDispatch(obj){
		
		layer.confirm('确认删除吗?',{btn:['是','否']},function(){
			 var url="<%=path %>/applyDispatch/deleteApplyDispatch.do?id="+obj;
