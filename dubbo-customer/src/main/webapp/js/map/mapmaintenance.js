...
function initMap(){
   ...
   //     alert(e.overlay.getPath());//线的轨迹数组
    ...
    var drawingManager = new BMapLib.DrawingManager(map, {
        ...
        enableDrawingTool: check, //是否显示工具栏
        drawingToolOptions: {
...
function drawPointGet(pointdata,imgurl){
	...
	//	map.openInfoWindow(infoWindow,point); //开启信息窗口
		...
		if(pointdata.TYPE==1){
     var url=basUrl+"/mapMaintenance/findNodeScada.do?code="+pointdata.CODE;
     layer_show('实时工况',url,'800','240');
		}else{
			var url=basUrl+"/mapMaintenance/findFeedScada.do?code="+pointdata.CODE;
			layer_show('实时工况',url,'600','200');
		}
